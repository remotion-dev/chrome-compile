...
# =============================================================================
# launch-ec2-x86.sh — Launch an x86_64 EC2 instance for Chromium compilation
#
# Region:    eu-central-1
...
VOLUME_SIZE=200
KEY_NAME="${KEY_NAME:-chrome-build-key-x86}"
...
INSTANCE_ID=$(aws ec2 run-instances \
  ...
  --region "$REGION" \
  --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=chrome-build-linux-x86}]' \
...
echo "Then run:"
echo "  sudo -i"
echo "  # copy over setup-ec2-x86.sh, Dockerfile-x86, build-chromium-x86.sh, .gclient, args-x86.gn"
echo "  bash setup-ec2-x86.sh"
